...
function printAllQuizzes(response) {
    ...
    <button onclick="habilitarCriacaoQuizz()" data-identifier="create-quizz">
      <h2>Criar Quizz</h2>
    </button>
  </article>
  ...
  <section onclick="habilitarCriacaoQuizz()" class="you_quizzes hidden">
        ...
        <img onclick="habilitarCriacaoQuizz()" src="/media/add_button.png" data-identifier="create-quizz" alt="Criar um novo Quizz">
</section><div class="fatherM"><div class=mediaQ>`

            let userQuizTemplate = `<article class="quizz_box quiz" id="${quiz.id}" onclick="openQuiz(this)" data-identifier="user-quizzes" data-identifier="quizz-card">
    ...
    allQuizzesEl.innerHTML += `</div></div>`
            ...
            let quizTemplate = `<article class="quizz_box quiz" id="${id}" onclick="openQuiz(this)" data-identifier="general-quizzes" data-identifier="quizz-card"><img src="${image}" alt="Quizz image" class="image_quizz"><h1 class="titulo_quizz">${title}</h1></article>`
...
function displayQuiz(response) {
    ...
    questions.forEach(question => {
        ...
        answers.forEach(answer => {
            ...
            <img class="img_option" src=${answer.image} data-identifier="answer" alt=""><p class="name_option">${answer.text}</p></div>`
        })

        templateQuizQuestions += `<nav class="full_question_box" id="${questions.indexOf(question)}">
        <div class="pergunta_box"><h1>${question.title} data-identifier="question" </h1></div><div class="options_box">${templateQuizAnswers}</div></nav>`
...
function showResult(correctAnswerPercentage) {
    ...
    levels.forEach(level => {
        if (correctAnswerPercentage >= level.minValue) {
            templateResult = `
            <div class="percentage_box"><h1>${correctAnswerPercentage}% de acerto: ${level.title} data-identifier="quizz-result"</h1></div>
